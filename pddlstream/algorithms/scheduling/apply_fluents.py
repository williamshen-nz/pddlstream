...
def get_steps_from_stream(stream_plan, step_from_fact, node_from_atom):
    ...
    for result in reversed(stream_plan):
        ...
        for fact in result.instance.get_domain():
            step_from_fact[fact] = (
                step_from_fact.get(fact, set()) | steps_from_stream[result]
            )
...
def get_fluent_instance(external, input_objects, state):
    ...
    fluent_facts = map(
        fact_from_fd,
        filter(
            lambda f: isinstance(f, pddl.Atom) and (f.predicate in external.fluents),
            state,
        ),
    )
...
def convert_fluent_streams(
    ...
    steps_from_stream = get_steps_from_stream(
        stream_plan, step_from_fact, node_from_atom
    )
    ...
    for result in stream_plan:
        ...
        if (
            ...
            or (result.opt_index != 0)
            or (not external.is_fluent)
        ):
        ...
        if outgoing_edges[result]:
            ...
            raise NotImplementedError(
                "Fluent stream is required for another stream: {}".format(result)
            )
        ...
        for state_index in steps_from_stream[result]:
            ...
            if new_output_objects and (state_index <= len(action_plan) - 1):
                ...
                instance.var_mapping = {
                    p: output_mapping.get(v, v) for p, v in instance.var_mapping.items()
                }
            new_instance = get_fluent_instance(
                external, result.instance.input_objects, real_states[state_index]
            )
            # TODO: handle optimistic here
            new_result = new_instance.get_result(
                new_output_objects, opt_index=result.opt_index
            )
