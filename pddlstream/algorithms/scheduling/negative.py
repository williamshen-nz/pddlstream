...
from pddlstream.algorithms.scheduling.reinstantiate import (
    reinstantiate_action_instances,
    reinstantiate_axiom_instances,
)
...
def get_negative_result(negative, input_objects, fluent_facts=frozenset()):
    ...
    return instance._Result(
        ...
        call_index=instance.num_calls,
        optimistic=optimistic,
    )


def convert_negative_stream(
    ...
    if negative.is_fluent:
        ...
        for step in step_from_atom[literal]:
            fluent_facts_list.append(
                list(
                    map(
                        fact_from_fd,
                        filter(
                            lambda f: isinstance(f, pddl.Atom)
                            and (f.predicate in negative.fluents),
                            real_states[step],
                        ),
                    )
                )
...
def recover_negative_axioms(
    ...
    for axiom_plan, action_instance in safe_zip(axiom_plans, action_plan):
        preimage = [
            l
            for l in plan_preimage(axiom_plan + [action_instance])
        ...
        negative_axiom_plan = extract_axiom_plan(
            opt_task, preimage, negative_from_name, static_state=opt_task.init
        )
