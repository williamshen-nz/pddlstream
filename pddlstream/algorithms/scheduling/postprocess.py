...
def reschedule_stream_plan(
    ...
    reschedule_problem = get_problem(
        evaluations, goal_expression, domain, unit_costs=False
    )
    ...
    stream_names, effort = solve_from_task(
        sas_task,
        planner=planner,
        max_planner_time=max_reschedule_time,
        max_cost=max_effort,
        debug=debug,
    )
...
def shorten_stream_plan(evaluations, stream_plan, target_facts):
    ...
    evaluation_subgoals = set(
        filter(evaluations.__contains__, map(evaluation_from_fact, all_subgoals))
    )
    open_subgoals = set(
        filter(lambda f: evaluation_from_fact(f) not in evaluations, all_subgoals)
    )
    results_from_fact = {}
    ...
    for removed_result in reversed(stream_plan):  # TODO: only do in order?
        ...
        if all(2 <= len(results_from_fact[fact]) for fact in certified_subgoals):
            node_from_atom = get_achieving_streams(
                evaluation_subgoals, set(stream_plan) - {removed_result}
            )
...
def postprocess_stream_plan(evaluations, domain, stream_plan, target_facts):
    ...
    if DO_RESCHEDULE:
        ...
        new_stream_plan = reschedule_stream_plan(
            evaluations, target_facts, domain, stream_plan
        )
