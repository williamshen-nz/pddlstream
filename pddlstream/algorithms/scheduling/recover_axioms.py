...
def get_necessary_axioms(conditions, axioms, negative_from_name):
    ...
    def add_literals(literals):
        for lit in literals:
            ...
            if atom not in processed_atoms:
                atom_queue.append(
                    atom
                )  # Previously was lit.positive() for some reason?
    ...
    while atom_queue:
        ...
        for axiom in axioms_from_name[literal.predicate]:
            ...
            effect = pddl.Effect(
                [], pddl.Truth(), pddl.conditions.Atom(axiom.name, effect_args)
            )
            free_parameters = [p for p in axiom.parameters if p.name not in var_mapping]
            new_action = pddl.Action(
                axiom.name,
                free_parameters,
                len(free_parameters),
                pddl.Conjunction(parts),
                [effect],
                None,
            )
...
def is_useful_atom(atom, conditions_from_predicate):
    ...
    for atom2 in conditions_from_predicate[atom.predicate]:
        if all(
            is_parameter(a2) or (a1 == a2) for a1, a2 in safe_zip(atom.args, atom2.args)
        ):
            return True
    return False
...
def extraction_helper(state, instantiated_axioms, goals, negative_from_name={}):
    ...
    for axiom in all_axioms:
        if axiom.effect in goals:
            ...
            if init_atom in axiom_init:
                raise RuntimeError(
                    'Bug introduced by new "downward" where both the positive and negative atoms '
                    "of literal {} are in the initial state".format(
                        init_atom.positive()
                    )
                )
    ...
    axiom_from_atom, _ = get_achieving_axioms(
        state | axiom_init, helpful_axioms + [goal_action], negative_from_name
    )
    axiom_plan = []  # Could always add all conditions
    success = extract_axioms(
        state | axiom_init, axiom_from_atom, goals, axiom_plan, negative_from_name
    )
    if not success:
...
def extract_axiom_plan(task, goals, negative_from_name, static_state=set()):
    ...
    axiom_from_action = get_necessary_axioms(
        derived_goals, task.axioms, negative_from_name
    )
    ...
    task.init = {
        atom for atom in task.init if is_useful_atom(atom, conditions_from_predicate)
    }
    ...
    with Verbose(verbose=False):
        model = build_model.compute_model(
            pddl_to_prolog.translate(task)
        )  # Changes based on init
    opt_facts = instantiate.get_fluent_facts(task, model) | (task.init - static_state)
    mock_fluent = MockSet(
        lambda item: (item.predicate in negative_from_name) or (item in opt_facts)
    )
    instantiated_axioms = instantiate_necessary_axioms(
        model, static_state, mock_fluent, axiom_from_action
    )
    axiom_plan = extraction_helper(
        task.init, instantiated_axioms, derived_goals, negative_from_name
    )
...
def backtrack_axioms(conditions, axioms_from_effect, visited_atoms):
    ...
    for atom in conditions:
        ...
        for axiom in axioms_from_effect[atom]:
            ...
            visited_axioms.extend(
                backtrack_axioms(axiom.condition, axioms_from_effect, visited_atoms)
            )
...
def recover_axioms_plans(instantiated, action_instances):
    ...
    for action in new_action_instances + [get_goal_instance(instantiated.task.goal)]:
        all_conditions = list(get_precondition(action)) + list(
            flatten(cond for cond, _ in action.add_effects + action.del_effects)
        )
        ...
        for effects in [action.add_effects, action.del_effects]:
            ...
            for i, (conditions, effect) in reversed(list(enumerate(effects))):
                if all(
                    literal_holds(state, literal) or (literal in axiom_from_atom)
