...
from pddlstream.algorithms.instantiate_task import (
    instantiate_task,
    sas_from_instantiated,
    FD_INSTANTIATE,
)
from pddlstream.algorithms.scheduling.add_optimizers import (
    add_optimizer_effects,
    using_optimizers,
    recover_simultaneous,
)
from pddlstream.algorithms.scheduling.apply_fluents import convert_fluent_streams
from pddlstream.algorithms.scheduling.negative import (
...
from pddlstream.algorithms.scheduling.utils import (
    partition_results,
    add_unsatisfiable_to_goal,
    get_instance_facts,
)
...
OptSolution = namedtuple(
    "OptSolution", ["stream_plan", "opt_plan", "cost"]
)  # TODO: move to the below
...
def get_plan_cost(action_plan, cost_from_action):
    ...
    scaled_cost = sum([0.0] + [cost_from_action[instance] for instance in action_plan])
...
def instantiate_optimizer_axioms(instantiated, domain, results):
    ...
    with Verbose():
        ...
        new_instantiated = instantiate_task(
            task_from_domain_problem(temp_domain, temp_problem),
            use_fd=use_fd,
...
def recover_stream_plan(
    ...
    negative_from_name = {
        ...
        for external in negative
        if external.is_negated
    }
    ...
    last_from_fact = {
        fact: min(steps)
        for fact, steps in steps_from_fact.items()
    ...
    curr_evaluations = evaluations_from_stream_plan(
        evaluations, stream_plan, max_effort=None
    )
    ...
    stream_plan = postprocess_stream_plan(
        evaluations, domain, stream_plan, last_from_fact
    )
    node_from_atom = get_achieving_streams(evaluations, stream_plan, max_effort=None)
    fact_sequence = [set(result.get_domain()) for result in stream_plan] + [
        extraction_facts
    ]
        ...
        for fact in facts:  # could flatten instead
            ...
            for domain_fact in result.instance.get_domain():
                last_from_fact[domain_fact] = min(
                    last_from_stream[result], last_from_fact.get(domain_fact, INF)
                )
    ...
    for result in stream_plan:
        if (last_from_stream[result] == 0) or not result.is_deferrable(
            ...
            for ancestor in get_ancestors(result, partial_orders) | {result}:
                ...
                if isinstance(ancestor, StreamResult):
                    bound_objects.update(
                        out for out in ancestor.output_objects if out.is_unique()
                    )
    ...
    for step, instance in enumerate(action_plan):
        for result in list(remaining_results):
            ...
            if conditions_hold(state, map(fd_from_fact, domain)):
                ...
                certified = {
                    fact for fact in result.get_certified() if get_prefix(fact) != EQ
                }
    ...
    eager_plan = convert_fluent_streams(
        eager_plan, real_states, action_plan, steps_from_fact, node_from_atom
    )
    combined_plan = []
    ...
    preimage_facts = {
        fact_from_fd(l) for l in full_preimage if (l.predicate != EQ) and not l.negated
    }
    ...
    for result in eager_plan:
        ...
        )  # Some facts might not be in the preimage
...
def solve_optimistic_sequential(
    domain,
    stream_domain,
    applied_results,
    all_results,
    ...
    **kwargs
    ...
    temporal_plan = None
    problem = get_problem(
        opt_evaluations, goal_expression, stream_domain
    )  # begin_metric
    ...
    action_instances = [
        action_from_name[
            name if RENAME_ACTIONS else "({} {})".format(name, " ".join(args))
        ]
...
def plan_streams(
    ...
    **kwargs
    ...
    if reachieve and not using_optimizers(all_results):
        achieved_results = {
            n.result for n in evaluations.values() if isinstance(n.result, Result)
        }
        init_evaluations = {
            e for e, n in evaluations.items() if n.result not in achieved_results
        }
    ...
    opt_evaluations = {
        evaluation_from_fact(f): n.result for f, n in node_from_atom.items()
    }
    ...
    optimistic_fn = (
        solve_optimistic_temporal if temporal else solve_optimistic_sequential
    )
    ...
    action_plan = transform_plan_args(
        map(pddl_from_instance, action_instances), obj_from_pddl
    )
    replan_step = min(
        [
            step + 1
            for step, action in enumerate(action_plan)
            if action.name in replan_actions
        ]
        or [len(action_plan) + 1]
    )  # step after action application

    stream_plan, opt_plan = recover_stream_plan(
        evaluations,
        stream_plan,
        opt_evaluations,
        goal_expression,
