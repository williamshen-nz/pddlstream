...
class SolutionStore(object):
    ...
    def is_timeout(self):
        return (self.max_time <= self.elapsed_time()) or not check_memory(
            self.max_memory
        )
...
def add_fact(evaluations, fact, result=INIT_EVALUATION, complexity=0):
    ...
    if (evaluation not in evaluations) or (
        complexity < evaluations[evaluation].complexity
    ):
...
def compute_complexity(evaluations, facts, complexity_op=COMPLEXITY_OP):
    ...
    return complexity_op(
        evaluations[evaluation_from_fact(fact)].complexity for fact in facts
    )
...
def stream_plan_complexity(
    evaluations, stream_plan, stream_calls, complexity_op=COMPLEXITY_OP
):
    ...
    # optimistic_facts = {}
    optimistic_facts = {
        fact: evaluations[evaluation_from_fact(fact)].complexity
    ...
    for i, result in enumerate(stream_plan):
        ...
        result_complexity = complexity_op(
            [0]
            + [
                optimistic_facts[fact]
