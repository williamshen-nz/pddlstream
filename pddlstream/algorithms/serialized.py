...
def apply_actions(domain, state, plan, unit_costs=False):
    ...
    fluents = get_fluents(domain)
    fluent_state = [
        value_from_evaluation(evaluation_from_fd(atom))
...
def solve_serialized(
    ...
    **kwargs
    ...
    static_init, _ = partition_facts(
        domain, init
    )  # might not be able to reprove static_int
    ...
    for i in range(len(goals)):
        ...
        local_problem = PDDLProblem(
            domain_pddl, constant_map, streams, None, state, goal
        )
        ...
        if retain_facts:
            ...
            state = (
                static_init + fluent_facts + local_certificate.preimage_facts
