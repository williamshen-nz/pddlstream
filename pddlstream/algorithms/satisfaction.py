...
from pddlstream.algorithms.disable_skeleton import (
    create_disabled_axioms,
    extract_disabled_clusters,
)
from pddlstream.algorithms.downward import make_domain, make_predicate, add_predicate
from pddlstream.algorithms.recover_optimizers import (
...
SatisfactionProblem = namedtuple(
    "SatisfactionProblem", ["stream_pddl", "stream_map", "init", "terms"]
)
...
def parse_value(value):
    return (
        OptimisticObject.from_opt(value, value)
        if is_parameter(value)
        else Object.from_value(value)
    )
...
def get_parameters(goal_facts):
    return {
        o for f in goal_facts for o in get_args(f) if isinstance(o, OptimisticObject)
    }
...
def extract_streams(evaluations, externals, goal_facts):
    ...
    for fact in goal_facts:
        ...
        retrace_instantiation(
            fact, streams, evaluations, free_parameters, visited_facts, stream_results
        )
...
def dump_assignment(solution):
    ...
    print(
        "Fact counts: {}".format(
            str_from_object(Counter(map(get_prefix, evaluations.all_facts)))
        )
    )  # preimage_facts
    if bindings is None:
        return
...
def constraint_satisfaction(
    ...
    **search_args
    ...
    goal_facts = set(
        filter(lambda f: evaluation_from_fact(f) not in evaluations, constraints)
    )
    free_parameters = sorted(get_parameters(goal_facts))
    print("Parameters:", free_parameters)

    externals = parse_stream_pddl(
        stream_pddl, stream_map, stream_info, unit_efforts=unit_efforts
    )
    ...
    store = SolutionStore(
        evaluations, max_time=max_time, success_cost=success_cost, verbose=verbose
    )
    ...
    while not store.is_terminated():
        ...
        if len(queue.skeletons) < max_skeletons:
            ...
            stream_plan = reschedule_stream_plan(
                init_evaluations,
                goal_facts,
                domain,
                stream_results,
        ...
        print(
            "Stream plan ({}, {:.3f}): {}".format(
                get_length(external_plan),
                compute_plan_effort(external_plan),
                external_plan,
            )
        )
    ...
    action_plan, cost, facts = revert_solution(
        store.best_plan, store.best_cost, evaluations
    )
