...
def create_disable_axiom(external_plan, use_parameters=True):
    ...
    component_plan = list(flatten(r.get_components() for r in stream_plan[:-1])) + list(
        unsatisfiable
    )
    ...
    optimistic_objects = {
        o for f in constraints for o in get_args(f) if isinstance(o, OptimisticObject)
...
def compute_failed_indices(skeleton):
    ...
    for binding in skeleton.root.post_order():
        ...
        if (
            (result is not None)
            and result.instance.num_calls
            and (not result.instance.successful)
        ):
...
def current_failed_cluster(binding):
    ...
    successful_results = [
        result
        for i, result in enumerate(binding.skeleton.stream_plan)
...
def extract_disabled_clusters(queue, full_cluster=False):
    ...
    for skeleton in queue.skeletons:
        ...
        if not binding.is_fully_bound:
            ...
            cluster_plans = (
                current_failed_cluster(binding)
                if full_cluster
                else current_failure_contributors(binding)
            )
