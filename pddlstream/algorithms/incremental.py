...
from pddlstream.language.attachments import (
    has_attachments,
    compile_fluents_as_attachments,
    solve_pyplanners,
)
...
def solve_sequential(
    evaluations, goal_exp, domain, unit_costs=False, debug=False, **search_args
):
...
def process_instance(
    ...
    for evaluation in add_facts(
        evaluations, new_facts, result=UNKNOWN_EVALUATION, complexity=fact_complexity
    ):
...
def process_stream_queue(instantiator, store, complexity_limit=INF, verbose=False):
    ...
    while (
        ...
        and instantiator
        and (instantiator.min_complexity() <= complexity_limit)
    ):
    ...
    if verbose:
        print(
            "Eager Calls: {} | Successes: {} | Results: {} | Counts: {}".format(
                len(instances),
                num_successes,
                len(results),
                str_from_object(
                    Counter(instance.external.name for instance in instances)
                ),
            )
        )
...
def solve_incremental(
    ...
    **search_kwargs
    ...
    num_calls += process_stream_queue(
        instantiator, store, complexity_limit, verbose=verbose
    )
    while (
        not store.is_terminated()
...
def solve_immediate(problem, **kwargs):
    ...
    return solve_incremental(
        problem, start_complexity=0, complexity_step=0, max_complexity=0, **kwargs
    )
...
def solve_exhaustive(problem, **kwargs):
    ...
    """
    return solve_incremental(
        problem, start_complexity=INF, complexity_step=INF, max_complexity=INF, **kwargs
    )
