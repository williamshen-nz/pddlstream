...
from pddlstream.algorithms.common import (
    is_instance_ready,
    compute_complexity,
    stream_plan_complexity,
    add_certified,
...
from pddlstream.algorithms.reorder import (
    get_output_objects,
    get_object_orders,
    get_partial_orders,
    get_initial_orders,
...
Priority = namedtuple(
    "Priority", ["not_greedy", "complexity", "visits", "remaining", "cost"]
)  # TODO: FIFO
...
def compute_affected_downstream(stream_plan, index):
    ...
    for index2 in range(index + 1, len(stream_plan)):
        ...
        if (
            output_objects & result2.instance.get_all_input_objects()
        ):  # TODO: get_object_orders
            output_objects.update(
                get_output_objects(result2)
            )  # TODO: just include directly affected?
...
class Skeleton(object):
    def __init__(self, queue, stream_plan, action_plan, cost):
        ...
        self.affected_indices = [
            compute_affected_downstream(self.stream_plan, index)
            for index in range(len(self.stream_plan))
        ]

        stream_orders = get_partial_orders(
            self.stream_plan
        )  # init_facts=self.queue.evaluations)
        index_from_result = get_mapping(stream_plan, range(len(stream_plan)))
        index_orders = {
            (index_from_result[r1], index_from_result[r2]) for r1, r2 in stream_orders
        }

        preimage = stream_plan_preimage(stream_plan)
        self.preimage_complexities = [
            [
                queue.evaluations[evaluation_from_fact(fact)].complexity
                for fact in stream.get_domain()
                if fact in preimage
            ]
    ...
    def compute_complexity(self, stream_calls, complexities=[]):
        ...
        for index in range(start_index, len(self.stream_plan)):
            complexities[index] = self.compute_index_complexity(
                index, stream_calls[index], complexities
            )
        return complexities
    ...
    def compute_index_complexity(self, index, num_calls, complexities):
        ...
        domain_complexity = COMPLEXITY_OP(
            [0]
            + self.preimage_complexities[index]
            + [complexities[index2] for index2 in self.incoming_indices[index]]
        )
        return domain_complexity + self.stream_plan[index].external.get_complexity(
            num_calls=num_calls
        )

    def update_best(self, binding):
        if (
            (self.best_binding is None)
            or (self.best_binding.index < binding.index)
            or (
                (self.best_binding.index == binding.index)
                and (binding.cost < self.best_binding.cost)
            )
        ):
    ...
    def bind_stream_result(self, index, mapping):
        return self.stream_plan[index].remap_inputs(
            mapping
        )  # Has optimistic output objects
    ...
    def visualize_bindings(self):
        # TODO: remap outputs
        orders = {
            (binding1.parent_result, binding2.parent_result)
...
class Binding(object):
    ...
    def result(self):
        if self._result is False:
            ...
            if not self.is_fully_bound:
                self._result = self.skeleton.bind_stream_result(
    ...
    def compute_complexity(self):
        ...
        if self.complexity is None:
            full_history = self.history + [
                self.calls
            ]  # TODO: relevant history, full history, or future
            future = full_history + [0] * (
                len(self.skeleton.stream_plan) - len(full_history)
            )
            parent_complexities = (
                [0] * len(self.skeleton.stream_plan)
                if self.index == 0
                else self.parent.complexities
            )
            if self.skeleton.outgoing_indices[self.index]:
                self.complexities = self.skeleton.compute_complexity(
                    future, complexities=parent_complexities[: self.index]
                )
            else:
                self.complexities = list(parent_complexities)
                self.complexities[self.index] = self.skeleton.compute_index_complexity(
                    self.index, self.calls, self.complexities
                )
    ...
    def check_complexity(self, complexity_limit=INF):
        if complexity_limit == INF:
            return True
        if any(
            calls > complexity_limit for calls in [self.max_history, self.calls]
        ):  # + self.history
    ...
    def check_downstream_helper(self, affected):
        ...
        if (
            self.is_unsatisfied()
        ):  # or type(self.result) == FunctionResult): # not self.visits
        ...
        return any(
            binding.check_downstream_helper(affected) for binding in self.children
        )
    ...
    def get_priority(self):
        if not USE_PRIORITIES:
            return Priority(
                not_greedy=True, complexity=0, visits=self.visits, remaining=0, cost=0.0
            )
        ...
        priority = self.compute_complexity() + (
            self.visits - self.calls
        )  # TODO: check this
        # TODO: call_index
        remaining = len(self.skeleton.stream_plan) - self.index
        return Priority(
            not self.is_greedy(), priority, self.visits, remaining, self.cost
        )
    ...
    def update_bindings(self):
        ...
        for call_idx in range(self.calls, instance.num_calls):
            for new_result in instance.results_history[
                call_idx
            ]:  # TODO: don't readd if successful already
                if new_result.is_successful():
                    new_bindings.append(
                        Binding(
                            ...
                            mapping=update_bindings(
                                self.mapping, self.result, new_result
                            ),
                            index=self.index
                            + 1,  # TODO: history instead of results_history
                            parent=self,
    ...
    def __repr__(self):
        return "{}(skeleton={}, {})".format(
            self.__class__.__name__, self.skeleton.index, self.result
        )
...
class SkeletonQueue(Sized):
    ...
    def _process_binding(self, binding):
        ...
        if (
            REQUIRE_DOWNSTREAM and not binding.check_downstream()
        ):  # TODO: move check_complexity here
    ...
    def process_until_new(self, print_frequency=1.0):
        ...
        while self.is_active() and (not num_new):
            ...
            if readd is True:
                ...
                self.standby.append(
                    binding
                )  # TODO: test for deciding whether to standby
            num_new += is_new
            if print_frequency <= elapsed_time(last_time):
                print(
                    "Queue: {} | Iterations: {} | Time: {:.3f}".format(
    ...
    def process_complexity(self, complexity_limit):
        ...
        while self.is_active():
            _, binding = self.pop_binding()
            if binding.check_complexity(
                complexity_limit
            ):  # not binding.up_to_date() or
                readd, is_new = self._process_binding(binding)
                num_new += is_new
    ...
    def timed_process(self, max_time=INF, max_iterations=INF):
        ...
        if not self.is_active():
            return num_new
        print(
            "Sampling for up to {:.3f} seconds".format(max_time)
        )  # , max_iterations))
    ...
    def accelerate_best_bindings(self, **kwargs):
        ...
        for skeleton in self.skeletons:
            ...
            for result in skeleton.best_binding.recover_bound_results():
                ...
                add_certified(
                    self.evaluations, result, **kwargs
                )  # TODO: should special have a complexity of INF?
