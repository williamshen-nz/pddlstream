...
InstantiatedTask = namedtuple(
    "InstantiatedTask",
    ["task", "atoms", "actions", "axioms", "reachable_action_params", "goal_list"],
)
...
def instantiate_condition(action, is_static, args_from_predicate):
    ...
    atoms_from_cond = {
        condition: args_from_predicate[condition.predicate, get_constants(condition)]
...
def get_reachable_action_params(instantiated_actions):
    ...
    for inst_action in instantiated_actions:
        ...
        reachable_action_params[action].append(
            args
...
def instantiate_domain(task, prune_static=True):
    ...
    is_static = lambda a: isinstance(a, pddl.Atom) and (
        a.predicate not in fluent_predicates
    )
    ...
    for action in task.actions + task.axioms:
        for atom in filter(is_static, get_literals(get_precondition(action))):
            constants = tuple(
                (i, a) for i, a in enumerate(atom.args) if not is_parameter(a)
            )
    ...
    for atom in filter(
        is_static, task.init
    ):  # TODO: compute which predicates might involve constants
    ...
    for action in task.actions:
        for variable_mapping in instantiate_condition(
            action, is_static, args_from_predicate
        ):
    ...
    for axiom in task.axioms:
        for variable_mapping in instantiate_condition(
            axiom, is_static, args_from_predicate
        ):
            inst_axiom = axiom.instantiate(variable_mapping, init_facts, fluent_facts)
            if inst_axiom:
                instantiated_axioms.append(inst_axiom)

    reachable_facts, reachable_operators = get_achieving_axioms(
        init_facts, instantiated_actions + instantiated_axioms
    )
    atoms = {
        atom.positive()
        for atom in (init_facts | set(reachable_facts))
        if isinstance(atom, pddl.Literal)
    }
    relaxed_reachable = all(
        literal_holds(init_facts, goal) or goal in reachable_facts
        for goal in instantiate_goal(task.goal)
    )
    reachable_actions = [
        action
        for action in reachable_operators
        if isinstance(action, pddl.PropositionalAction)
    ]
    reachable_axioms = [
        axiom
        for axiom in reachable_operators
...
def dump_instantiated(instantiated):
    print(
        ...
        "Axioms: {}".format(
            ...
            str_from_object(
                Counter(action.action.name for action in instantiated.actions)
            ),
...
def sas_from_pddl(task, debug=False):
    ...
    with Verbose(debug):
        ...
        sas_task.metric = (
            task.use_min_cost_metric
        )  # TODO: are these sometimes not equal?
    return sas_task
...
def convert_instantiated(instantiated_task):
    ...
    return InstantiatedTask(
        task, init, actions, axioms, reachable_action_params, goal_list
    )  # init instead of atoms
