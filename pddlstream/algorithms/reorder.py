...
def get_object_orders(stream_plan):
    ...
    for i, stream1 in enumerate(stream_plan):
        for stream2 in stream_plan[i + 1 :]:
            if (
                set(get_output_objects(stream1))
                & stream2.instance.get_all_input_objects()
            ):
...
def compute_pruning_orders(
    ...
    dominates = lambda v1, v2: all(
        s1 <= s2 for s1, s2 in safe_zip(stats_fn(v1), stats_fn(v2))
...
def dynamic_programming(
    ...
    **kwargs
    ...
    priority_ordering = topological_sort(vertices, effort_orders)[
        ::-1
    ...
    while queue:  # searches backward from last to first
        ...
        for v in priority_ordering:  # most expensive first
            ...
            if (
                (v not in subset)
                and valid_head_fn(v, subset)
                and not (out_priority_orders[v] & applied)
            ):
                applied.add(v)
                ...
                subproblem = Subproblem(
                    cost=new_cost, head=v, subset=subset
...
def compute_distances(stream_plan):
    ...
    sources = {
        stream for stream in stream_plan if not in_stream_orders[stream]
    }  # In the reversed DAG
    ...
    for stream in stream_plan:
        if stream not in distances:
            distances[stream] = min(
                [max_distance] + [distances[s] - 1 for s in out_stream_orders[stream]]
            )
...
def layer_reorder_stream_plan(stream_plan, **kwargs):
    ...
    priority_fn = lambda s: Score(
        not s.external.has_outputs, distances[s], -s.stats_heuristic()
    )
    reverse_order = topological_sort(
        stream_plan, reversed_orders, priority_fn=priority_fn
    )
...
def compute_statistics(stream_plan, bias=True):
    stats_from_stream = {
        result: result.external.get_statistics() for result in stream_plan
    }
    if not bias:
...
def optimal_reorder_stream_plan(store, stream_plan, stats_from_stream=None, **kwargs):
    ...
    stream_orders = get_partial_orders(stream_plan)
    stream_orders = {
        (index_from_stream[s1], index_from_stream[s2]) for s1, s2 in stream_orders
    }
    # nodes = stream_plan
...
def reorder_stream_plan(store, stream_plan, algorithm=None, **kwargs):
    ...
    if algorithm == "optimal":
        ...
        return optimal_reorder_stream_plan(
            store, stream_plan, stats_from_stream, **kwargs
        )
