...
def find_build(fd_path):
    ...
    raise RuntimeError(
        "Please compile FastDownward first [.../pddlstream$ ./downward/build.py]"
    )
...
import normalize
import pddl_parser
from pddl_parser.parsing_functions import (
    parse_domain_pddl,
    parse_task_pddl,
    parse_condition,
    ...
    SEARCH_OPTIONS.update(
        {
            ...
            "ff-wastar{w}".format(
                w=w
            ): '--heuristic "h=ff(transform=adapt_costs(cost_type=PLUSONE))" '
            '--search "lazy_wastar([h],preferred=[h],reopen_closed=true,boost=100,w={w},'
            'preferred_successors_first=true,cost_type=PLUSONE,max_time=%s,bound=%s)"'.format(
                w=w
            ),
            "cea-wastar{w}".format(
                w=w
            ): '--heuristic "h=cea(transform=adapt_costs(cost_type=PLUSONE))" '
            ...
            "ff-astar{w}".format(
                w=w
            ): '--evaluator "h=ff(transform=adapt_costs(cost_type=PLUSONE))" '
...
if USE_CERBERUS:
    ...
    SEARCH_OPTIONS["cerberus"] = " ".join(
        s if s.startswith("--") else '"{}"'.format(s) for s in mod.config_string()
    )
...
def parse_problem(domain, problem_pddl):
    ...
    args = list(
        parse_task_pddl(
            parse_lisp(problem_pddl), domain.type_dict, domain.predicate_dict
        )
    ) + [problem_pddl]
    return Problem(*args)
...
def parse_goal(goal_exp, domain):
    ...
    return parse_condition(
        pddl_list_from_expression(goal_exp), domain.type_dict, domain.predicate_dict
    ).simplified()
...
def get_problem(evaluations, goal_exp, domain, unit_costs=False):
    ...
    typed_objects = list(
        {make_object(pddl_from_object(obj)) for obj in objects} - set(domain.constants)
    )
    ...
    if USE_FORBID:
        problem_pddl = get_problem_pddl(
            evaluations, goal_exp, domain.pddl, temporal=False
        )
...
def task_from_domain_problem(domain, problem, add_identical=True):
    ...
    requirements = pddl.Requirements(
        sorted(set(domain.requirements.requirements + task_requirements.requirements))
    )
...
def run_search(
    ...
    command = search.format(
        temp_dir + SEARCH_OUTPUT, planner_config, temp_dir + TRANSLATE_OUTPUT
    )
    ...
    proc = subprocess.Popen(
        command, stdout=subprocess.PIPE, shell=True, cwd=None, close_fds=True
    )
    ...
    if USE_FORBID:
        for filename in os.listdir(FORBID_PATH):
            if filename.startswith(SEARCH_OUTPUT):
                os.rename(
                    os.path.join(FORBID_PATH, filename),
...
def get_precondition(operator):
    if isinstance(operator, pddl.Action) or isinstance(
        operator, pddl.PropositionalAction
    ):
        return operator.precondition
    elif isinstance(operator, pddl.Axiom) or isinstance(
        operator, pddl.PropositionalAxiom
    ):
...
def substitute_derived(axiom_plan, action_instance):
    ...
    action_instance.precondition = list(
        (set(action_instance.precondition) | axiom_pre) - axiom_eff
    )
...
def make_domain(constants=[], predicates=[], functions=[], actions=[], axioms=[]):
    ...
    return Domain(
        ...
        requirements=pddl.Requirements([]),
        types=types,
        type_dict={ty.name: ty for ty in types},
        constants=constants,
