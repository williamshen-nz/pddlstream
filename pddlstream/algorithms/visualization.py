...
def log_plans(stream_plan, action_plan, iteration):
    ...
    with open(PLAN_LOG_FILE, "a+") as f:
        f.write(
            ...
            "Stream plan: {}\n"
            "Action plan: {}\n\n".format(
                iteration, decomposed_plan, stream_plan, str_from_plan(action_plan)
            )
        )
...
def create_synthesizer_visualizations(result, iteration):
    ...
    visualize_constraints(
        result.get_objectives(), os.path.join(CONSTRAINT_NETWORK_DIR, filename)
    )
    visualize_stream_plan_bipartite(
        stream_plan, os.path.join(STREAM_PLAN_DIR, filename)
    )
...
def create_visualizations(evaluations, stream_plan, iteration):
    ...
    for stream in stream_plan:
        constraints.update(
            filter(
                lambda f: evaluation_from_fact(f) not in evaluations,
    ...
    if len(decomposed_plan) != len(stream_plan):
        visualize_stream_plan(
            decompose_stream_plan(stream_plan), os.path.join(STREAM_PLAN_DIR, filename)
        )

    # visualize_stream_plan_bipartite(stream_plan, os.path.join(STREAM_PLAN_DIR, 'fused_' + filename))
    visualize_stream_plan(
        stream_plan, os.path.join(STREAM_PLAN_DIR, "fused_" + filename)
    )
...
def visualize_stream_orders(
    orders, streams=[], filename="stream_orders" + DEFAULT_EXTENSION
):
    from pygraphviz import AGraph
...
def visualize_stream_plan(stream_plan, filename="stream_plan" + DEFAULT_EXTENSION):
    return visualize_stream_orders(
        get_partial_orders(stream_plan), streams=stream_plan, filename=filename
    )
...
def visualize_stream_plan_bipartite(
    stream_plan, filename="stream_plan" + DEFAULT_EXTENSION, use_functions=False
):
    from pygraphviz import AGraph
    ...
    def add_stream(stream):
        ...
        s_stream = (
            str(stream.instance) if isinstance(stream, FunctionResult) else str(stream)
        )
