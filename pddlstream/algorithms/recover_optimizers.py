...
def combine_optimizer_plan(stream_plan, functions):
    ...
    function_plan = list(
        filter(
            lambda r: get_prefix(r.instance.external.head) in optimizer.objectives,
            functions,
        )
    )
    external_plan = stream_plan + function_plan
    cluster_plans = (
        get_stream_plan_components(external_plan) if CLUSTER else [external_plan]
    )
    optimizer_plan = []
    for cluster_plan in cluster_plans:
        ...
        instance = stream.get_instance(
            stream.input_objects, fluent_facts=stream.fluent_facts
        )
...
def combine_optimizers(evaluations, external_plan):
    ...
    optimizers = {
        get_optimizer(r) for r in stream_plan
    ...
    for result in combined_results:
        current_facts.update(
            filter(
                lambda f: evaluation_from_fact(f) in evaluations, result.get_domain()
            )
        )
    combined_plan = []
...
def retrace_instantiation(
    fact, streams, evaluations, free_parameters, visited_facts, planned_results
):
    ...
    for stream in streams:
        for cert in stream.certified:
            if get_prefix(fact) == get_prefix(cert):
                mapping = get_mapping(
                    get_args(cert), get_args(fact)
                )  # Should be same anyways
...
def replan_with_optimizers(evaluations, external_plan, domain, optimizers):
    ...
    initial_evaluations = {
        e: n for e, n in evaluations.items() if n.result == INIT_EVALUATION
    }
    ...
    optimizer_results = []
    for optimizer in {
        get_optimizer(r) for r in new_results
    }:  # None is like a unique optimizer:
    ...
    optimizer_plan = reschedule_stream_plan(
        initial_evaluations,
        goal_facts,
        copy.copy(domain),
