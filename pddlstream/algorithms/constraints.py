...
OrderedSkeleton = namedtuple(
    "OrderedSkeleton", ["actions", "orders"]
)  # TODO: AND/OR tree
...
class PlanConstraints(object):
    def __init__(self, skeletons=None, groups={}, exact=True, hint=False, max_cost=INF):
        ...
        if skeletons is not None:
            skeletons = [
                skeleton
                if isinstance(skeleton, OrderedSkeleton)
                else OrderedSkeleton(skeleton, linear_order(skeleton))
    ...
    def dump(self):
        print(
            "{}(exact={}, max_cost={})".format(
                self.__class__.__name__, self.exact, self.max_cost
            )
        )
...
def add_plan_constraints(constraints, domain, evaluations, goal_exp, internal=False):
    ...
    for num, skeleton in enumerate(constraints.skeletons):
        ...
        order_facts = [
            (order_predicate, to_obj("n{}".format(num)), to_obj("t{}".format(step)))
        ...
        for step, (name, args) in enumerate(actions):
            ...
            ancestors, descendants = get_ancestors(step, orders), get_descendants(
                step, orders
            )
            ...
            ancestor_parameters = parameters & set(
                filter(is_parameter, (p for idx in ancestors for p in actions[idx][1]))
            )
            ...
            parallel_parameters = parameters & set(
                filter(is_parameter, (p for idx in parallel for p in actions[idx][1]))
            )
            ...
            existing_preconditions = [
                (assigned_predicate, to_constant(p), local_from_global[p])
            ...
            group_preconditions = [
                (
                    group_predicate
                    if is_hashable(a) and (a in constraints.groups)
                    else EQ,
                    to_obj(a),
                    p,
                )
