...
class SynthStreamResult(StreamResult):
    ...
    def decompose(self):
        ...
        for i, stream in enumerate(self.instance.external.streams):
            ...
            input_objects = tuple(
                self.mapping[macro_from_micro[inp]] for inp in stream.inputs
            )
            instance = stream.get_instance(input_objects)
            output_objects = tuple(
                self.mapping[macro_from_micro[out]] for out in stream.outputs
            )
...
class SynthStream(Stream):
    ...
    def __init__(
        ...
        super(SynthStream, self).__init__(
            synthesizer.name, gen_fn, inputs, domain, outputs, certified, info
        )
...
class StreamSynthesizer(Performance):  # JointStream | Stream Combiner
    ...
    def get_synth_stream(self, stream_plan):
        ...
        (
            ...
            output_objects,
            fluent_facts,
        ) = get_cluster_values(stream_plan)
        if fluent_facts:
            raise NotImplementedError()
        mega_stream = SynthStream(
            self,
            inputs,
            domain,
            outputs,
...
def expand_cluster(synthesizer, v, neighbors, processed):
    ...
    while queue:
        ...
        for v2 in neighbors[v1]:
            if (v2 not in processed) and (
                v2.instance.external.name in synthesizer.streams
            ):
                cluster.add(v2)
                queue.append(v2)
                processed.add(v2)
    ...
    for result in stream_plan:  # Processing in order is important
        ...
        for synthesizer in synthesizers:
            ...
            new_stream_plan.extend(
                filter(lambda s: isinstance(s, FunctionResult), ordered_cluster)
            )
            break
        else:
