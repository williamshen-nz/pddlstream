...
def replace_derived(task, negative_init, action_instances):
    ...
    task.init = (set(task.init) | {a.negate() for a in negative_init}) - set(
        function_assignments
    )
    for instance in action_instances:
        ...
        with Verbose(
            False
        ):  # TODO: helpful_axioms prunes axioms that are already true (e.g. not Unsafe)
            helpful_axioms, axiom_init, _ = axiom_rules.handle_axioms(
                [instance], instantiated_axioms, goal_list
            )
        axiom_from_atom, _ = get_achieving_axioms(
            task.init | negative_init | set(axiom_init), helpful_axioms
        )
...
def get_combined_orders(evaluations, stream_plan, action_plan, domain):
    ...
    negative_results = filter(
        lambda r: isinstance(r, PredicateResult) and (r.value == False), stream_plan
    )
    negative_init = set(
        fd_from_evaluation(evaluation_from_fact(f))
    ...
    task = task_from_domain_problem(
        domain, get_problem(opt_evaluations, goal_expression, domain, unit_costs=True)
    )
