...
class FutureStream(Stream):
    def __init__(self, stream, static_domain, fluent_domain, static_certified):
        ...
        super(FutureStream, self).__init__(
            stream_name,
            stream.gen_fn,
            stream.inputs,
            static_domain,
...
def create_static_stream(stream, evaluations, fluent_predicates, future_fn):
    ...
    stream_atom = ("{}-result".format(stream.name),) + tuple(
        stream.inputs + stream.outputs
    )
...
def augment_evaluations(evaluations, future_map):
    for evaluation in list(filter(is_atom, evaluations)):
        ...
        if name in future_map:
            ...
            add_fact(
                evaluations,
                fact_from_evaluation(new_evaluation),
...
def compile_to_exogenous_actions(evaluations, domain, streams):
    ...
    for stream in list(streams):
        ...
        new_streams.append(
            create_static_stream(stream, evaluations, fluent_predicates, future_fn)
        )
        stream_atom = new_streams[-1].certified[0]
        add_predicate(
            domain, make_predicate(get_prefix(stream_atom), get_args(stream_atom))
        )
...
def compile_to_exogenous(evaluations, domain, streams):
    ...
    if not exogenous_predicates:
        return False
    print(
        "Warning! The following predicates are mentioned in both action effects "
