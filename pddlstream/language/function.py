...
def add_opt_function(
    ...
    **external_kwargs
    ...
    stream_fn = lambda *args, **kwargs: constant + coefficient * base_fn(
        *args, **kwargs
    )
...
class FunctionInfo(ExternalInfo):
    ...
    def __init__(
        self, opt_fn=None, eager=_default_eager, **kwargs
    ):  # Setting eager=True as a heuristic
...
class FunctionResult(Result):
    def __init__(self, instance, value, optimistic=True):
        super(FunctionResult, self).__init__(
            instance, opt_index=0, call_index=0, optimistic=optimistic
        )
...
class FunctionInstance(Instance):
    ...
    def __repr__(self):
        return "{}=?{}".format(
            str_from_head(self.head), self.external.codomain.__name__
        )
