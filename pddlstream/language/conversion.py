...
def dnf_from_positive_formula(parent):
    ...
    if prefix == AND:
        for combo in product(
            *(dnf_from_positive_formula(child) for child in parent[1:])
        ):
...
def list_from_conjunction(parent):
    ...
    if len(clauses) >= 2:
        raise ValueError(
            "Formula {} has more than one conjunctive clauses".format(parent)
        )
    return clauses[0]
...
def transform_plan_args(plan, fn):
    ...
    return list(
        filter(
            ...
            [transform_action_args(action, fn) for action in plan],
        )
    )
