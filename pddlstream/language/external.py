...
def get_defer_any_unbound(unique=False):
    def defer_any_unbound(result, bound_objects=set(), *args, **kwargs):
        ...
        if unique and not defer_unique(result):
            return False
        return not all(
            isinstance(obj, Object) or (obj in bound_objects)
...
class Result(object):
    ...
    def compute_complexity(self, evaluations, **kwargs):
        ...
        return compute_complexity(
            evaluations, self.get_domain(), **kwargs
    ...
    def success_heuristic(self):  # High is likely to succeed
        ...
        return Score(
            num_free, -len(self.external.inputs)
        )  # TODO: treat objects in the same domain as a unit
...
class Instance(object):
    ...
    def domain(self):
        if self._domain is None:
            ...
            self._domain = tuple(
                substitute_fact(atom, self.mapping) for atom in self.external.domain
            )
        return self._domain
...
class External(Performance):
    ...
    def __init__(self, name, info, inputs, domain):
        ...
        for p, c in Counter(self.inputs).items():
            if not is_parameter(p):
                ...
                raise ValueError(
                    "Input [{}] for stream [{}] is not a parameter".format(p, name)
                )
            if c != 1:
                raise ValueError(
        ...
        for p in parameters - set(self.inputs):
            raise ValueError(
                "Parameter [{}] for stream [{}] is not included within inputs".format(
                    p, name
                )
            )
        for p in set(self.inputs) - parameters:
            print(
                "Warning! Input [{}] for stream [{}] is not covered by a domain condition".format(
                    p, name
                )
            )
        self.constants = {
            a for i in self.domain for a in get_args(i) if not is_parameter(a)
        }
        self.instances = {}
    ...
    def overhead_heuristic(self):  # Low is little overhead
        ...
        return Score(
            self.is_fluent, not self.is_function, self.has_outputs, len(self.inputs)
        )  # structural/relational overhead
