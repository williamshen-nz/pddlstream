    ...
    def get_opt_gen_fn(self, instance):
        ...
        def gen_fn(*input_values):
            ...
            for idx in irange(instance.num_optimistic):  # self.num
                ...
                if unique:
                    outputs = tuple(
                        UniqueOptValue(instance, idx, out) for out in external.outputs
                    )
                else:
                    outputs = tuple(
...
class StreamInfo(ExternalInfo):
    def __init__(
        self, opt_gen_fn=None, negate=False, simultaneous=False, verbose=True, **kwargs
...
class StreamInstance(Instance):
    ...
    def wrap_optimistic(self, output_values, call_index):
        ...
        for name, value, rep in zip(
            self.external.outputs, output_values, representative_outputs
        ):
    ...
    def _create_opt_generator(self, opt_index=None):
        ...
        if self.opt_gens[opt_index] is None:
            self.opt_gens[opt_index] = BoundedGenerator(
                self.opt_gen_fns[opt_index](*self.get_input_values())
            )
...
class Stream(External):
    ...
    def __init__(
        ...
        if (
            ...
            and not (
                isinstance(self.info.opt_gen_fn, PartialInputs)
                and self.info.opt_gen_fn.unique
            )
        ):
