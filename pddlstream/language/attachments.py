...
def compile_fluents_as_attachments(domain, externals):
    state_streams = set(
        filter(lambda e: isinstance(e, Stream) and e.is_fluent, externals)
    )  # TODO: is_negated/is_special
    ...
    for action in domain.actions:
        for effect in action.effects:
            ...
            if any(
                literal.predicate in predicate_map
                for literal in get_literals(effect.condition)
            ):
                raise ValueError(
                    "Attachments cannot be in action effects: {}".format(effect)
                )
        action.attachments = {}
        preconditions = set()
        for literal in get_conjunctive_parts(action.precondition):
            ...
            preconditions.update(
                pddl.Atom(EQ, (mapping[out], PLACEHOLDER_OBJ.pddl))
                for out in stream.outputs
            )
            preconditions.update(
                fd_from_fact(substitute_fact(fact, mapping)) for fact in stream.domain
            )
...
def solve_pyplanners(
    instantiated, planner=None, max_planner_time=DEFAULT_MAX_TIME, max_cost=INF
):
    ...
    for action in instantiated.actions:
        ...
        py_action.test, fd_action_from_py_action[py_action] = get_attachment_test(
            action
        )
