        ...
        for fact in state_stream.certified:
            ...
            if predicate in predicate_map:
                ...
                raise NotImplementedError(
                    "Only one fluent stream can certify a predicate: {}".format(
                        predicate
                    )
                )
...
def remap_certified(literal, stream):
    ...
    if not all(
        arg in mapping for arg in stream.inputs
    ):  # Certified must contain all inputs
        return None
    return mapping
...
def compile_fluent_streams(domain, externals):
    state_streams = set(
        filter(lambda e: isinstance(e, Stream) and e.is_special, externals)
    )
    ...
    def fn(literal, action):
        ...
        if isinstance(action, pddl.Action):  # TODO: unified Action/Axiom effects
            for effect in action.effects:
                if isinstance(effect, pddl.Effect) and (
                    output_args & set(effect.literal.args)
                ):
                    raise RuntimeError(
                        "Fluent stream outputs cannot be in action effects: {}".format(
            ...
            axiom = action
            raise RuntimeError(
                "Fluent stream outputs cannot be in an axiom: {}".format(axiom.name)
            )

        blocked_args = safe_apply_mapping(stream.inputs, mapping)
        blocked_literal = literal.__class__(
            stream.blocked_predicate, blocked_args
        ).negate()
        if stream.is_negated:
            conditions = [blocked_literal]
            conditions.extend(
                pddl.Atom(
                    get_prefix(fact), safe_apply_mapping(get_args(fact), mapping)
                )  # fd_from_fact
                for fact in stream.domain
            )  # TODO: be careful when using imply
            return pddl.Conjunction(conditions)  # TODO: prune redundant conditions
        return pddl.Conjunction([literal, blocked_literal])

    for action in domain.actions:
        action.precondition = replace_literals(
            fn, action.precondition, action
        ).simplified()
        for effect in action.effects:
            effect.condition = replace_literals(
                fn, effect.condition, action
            ).simplified()
