...
def prune_dominated(collections):
    for i, collection1 in enumerate(collections):
        if all(
            (i == j) or not (collection2 <= collection1)
...
class VariableStream(ComponentStream):
    ...
    def __init__(self, optimizer, variables, inputs, domain, certified, infos):
        name = "{}-{}".format(
            optimizer.name, "-".join(map(get_parameter_name, variables))
        )
        ...
        if info is None:
            info = StreamInfo(
                opt_gen_fn=PartialInputs(unique=DEFAULT_UNIQUE),
                simultaneous=DEFAULT_SIMULTANEOUS,
            )
        super(VariableStream, self).__init__(
            optimizer, name, gen_fn, inputs, domain, variables, certified, info
        )
...
class ConstraintStream(ComponentStream):
    def __init__(self, optimizer, constraint, domain, infos):
        ...
        if info is None:
            info = StreamInfo(
                effort=get_effort_fn(optimizer.name), simultaneous=DEFAULT_SIMULTANEOUS
            )
        super(ConstraintStream, self).__init__(
            optimizer, name, gen_fn, inputs, domain, outputs, certified, info
        )
...
class OptimizerInstance(StreamInstance):
    ...
    def get_unsatisfiable(self):
        constraints = substitute_expression(
            self.external.certified, self.external.mapping
        )
        ...
        return [
            {result for index in cluster for result in result_from_index[index]}
...
class OptimizerStream(Stream):
    ...
    def __init__(self, optimizer, external_plan):
        ...
        (
            ...
            self.output_objects,
            self.fluent_facts,
        ) = get_cluster_values(external_plan)

        hint = self.create_hint()
        self.objectives = certified + functions
        gen_fn = get_list_gen_fn(
            optimizer.procedure, inputs, outputs, self.objectives, hint=hint
        )
        # assert len(self.get_cluster_plans()) == 1
        super(OptimizerStream, self).__init__(
            optimizer.name, gen_fn, inputs, domain, outputs, certified, optimizer.info
        )
    ...
    def create_hint(self):
        ...
        for result, mapping in safe_zip(self.stream_plan, self.macro_from_micro):
            if isinstance(result, StreamResult):
                for param, obj in safe_zip(
                    result.external.outputs, result.output_objects
                ):
    ...
    def mapping(self):
        return get_mapping(
            self.inputs + self.outputs, self.input_objects + self.output_objects
        )
...
def add_result_inputs(result, param_from_obj, local_mapping, inputs, input_objects):
    for param, obj in zip(
        result.instance.external.inputs, result.instance.input_objects
    ):
...
def get_cluster_values(stream_plan):
    ...
    for result in stream_plan:
        ...
        domain.update(
            set(substitute_expression(stream.domain, local_mapping)) - certified
        )
        if isinstance(result, PredicateResult):
            ...
            macro_from_micro.append(
                local_mapping
            )  # TODO: append for functions as well?
