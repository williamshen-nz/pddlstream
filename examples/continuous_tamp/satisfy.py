...
from examples.continuous_tamp.run import (
    pddlstream_from_tamp,
    display_plan,
    set_deterministic,
    TIGHT_SKELETON,
)
...
from pddlstream.algorithms.satisfaction import (
...
def main(success_cost=INF, use_costs=True):  # 0 | INF
    ...
    pddlstream_problem = pddlstream_from_tamp(
        tamp_problem, use_stream=not args.gurobi, use_optimizer=args.gurobi
    )
    ...
    with Profiler():
        if args.algorithm == "focused":
            ...
            solution = constraint_satisfaction(
                satisfaction_problem,
                stream_info=stream_info,
                costs=not args.unit,
                success_cost=success_cost,
