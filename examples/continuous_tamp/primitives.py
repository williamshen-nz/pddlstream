...
def rejection_sample_placed(
    block_poses={}, block_regions={}, regions={}, max_attempts=10
):
...
def interpolate(waypoints, velocity=1):
    ...
    differences = [0.0] + [
        np.linalg.norm(q2 - q1) for q1, q2 in zip(waypoints[:-1], waypoints[1:])
    ]
...
TAMPProblem = namedtuple(
    "TAMPProblem", ["initial", "regions", "goal_conf", "goal_regions", "goal_cooked"]
)
...
def mirror(n_blocks=1, n_robots=2):
    ...
    n_goals = n_blocks
    poses = [
        np.array([-(BLOCK_WIDTH + 1) * x - BLOCK_WIDTH, 0]) for x in range(n_blocks)
    ]
...
def blocked(n_blocks=3, n_robots=1, deterministic=True):
    ...
    if deterministic:
        ...
        poses.extend(
            np.array([lower + BLOCK_WIDTH / 2 + (BLOCK_WIDTH + 1) * x, 0])
        ...
        block_poses = rejection_sample_placed(
            block_regions=block_regions, regions=REGIONS
        )
...
def blocked_cost(n_blocks=2, n_robots=1):
    ...
    blocks = make_blocks(n_blocks)
    lower, upper = REGIONS[GROUND_NAME]
    poses = [np.zeros(2), np.array([goal_region[0] + 3 * BLOCK_WIDTH / 2, 0.0])]
    poses.extend(
        np.array([lower + BLOCK_WIDTH / 2 + (BLOCK_WIDTH + 1) * x, 0])
        for x in range(n_blocks - len(poses))
    )
...
def cook(n_blocks=2, n_goals=1, n_robots=1):
    ...
    block_poses = rejection_sample_placed(
        block_regions=initial_regions, regions=regions
    )
...
def get_value_at_time(traj, fraction):
    ...
    if len(waypoints) == 1:
        return waypoints[0]
    distances = [0.0] + [
        np.linalg.norm(np.array(q2) - np.array(q1))
        for q1, q2 in zip(waypoints, waypoints[1:])
    ]
    cum_distances = np.cumsum(distances)
    cum_fractions = np.minimum(
        cum_distances / cum_distances[-1], np.ones(cum_distances.shape)
    )
...
def update_state(state, action, t):
    ...
    if name == "move":
        ...
        if fraction < threshold:
            ...
            robot_confs[robot] = get_value_at_time(
                traj[::-1], (fraction - threshold) / (1 - threshold)
            )
    elif name == "place":
        robot, block, pose, grasp, conf = args
        traj = [conf, pose - grasp]
