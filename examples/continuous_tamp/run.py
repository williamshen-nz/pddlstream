...
def create_problem(tamp_problem, hand_empty=False, manipulate_cost=1.0):
    ...
    init = (
        ...
        + [
            ...
            or ((b in tamp_problem.goal_cooked) and (r in STOVE_NAMES))
        ]
    )
    ...
    goal_literals = [] + [
        ("Cooked", b) for b in tamp_problem.goal_cooked
    ]  # Placeable for the stove
...
def pddlstream_from_tamp(
    ...
    if use_optimizer:
        external_paths.append(
            get_file_path(__file__, "optimizer/optimizer.pddl")
...
def main():
    ...
    stream_info = {
        ...
        "s-ik": StreamInfo(
            defer_fn=get_defer_all_unbound(inputs="?g")
        ),  # defer_fn | defer_unbound
