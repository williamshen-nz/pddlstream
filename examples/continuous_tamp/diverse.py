...
def get_stream_value(value):
    ...
    return StreamValue(
        value.stream,
        tuple(map(get_stream_value, value.input_values)),
        value.output_parameter,
    )


def p_conjunction(stream_plans, stats_from_stream):
    return np.product(
        [stats_from_stream[stream].p_success for stream, _ in set.union(*stream_plans)]
    )
...
def main():
    ...
    parser.add_argument(
        "-a", "--algorithm", default="focused", help="Specifies the algorithm"
    )
    parser.add_argument(
        "-s",
        "--skeleton",
    ...
    viewer = ContinuousTMPViewer(
        SUCTION_HEIGHT, tamp_problem.regions, title="Continuous TAMP"
    )
    ...
    for order in permutations(blocks):
        for region in regions:  # Objects could also have separate goals
            ...
            skeleton = create_skeleton(
                robot, order, home=tamp_problem.goal_conf is not None
            )
            print(skeleton)
            skeletons = [skeleton]
            # skeletons = None
    ...
    for problems in combinations(streams_from_problem, r=2):  # Make r a parameter
        ...
        assert 0 <= p <= 1
        print(
            "Problems: {} | Intersection: {} | p={:.3f}".format(
                problems, len(intersection), p
            )
        )  # , intersection)
        if p > best_p:
