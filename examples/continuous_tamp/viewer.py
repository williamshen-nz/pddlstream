...
class ContinuousTMPViewer(object):
    def __init__(
        ...
        self.robot_dist = self.dist_height / 2.0
        ...
        self.dynamic.extend(
            [
                ...
                self.canvas.create_text(
                    self.scale_x(x), self.scale_y(y + height / 2), text=name
                ),
            ]
        )
    ...
    def draw_environment(self):
        ...
        for name, region in sorted(
            self.regions.items(), key=lambda pair: get_width(pair[1]), reverse=True
        ):
    ...
    def draw_robot(
        ...
        self.dynamic.extend(
            [
                self.canvas.create_rectangle(
                    self.scale_x(x - SUCTION_WIDTH / 2.0),
                    self.scale_y(y - self.suction_height / 2.0),
                    self.scale_x(x + SUCTION_WIDTH / 2.0),
                    self.scale_y(y + self.suction_height / 2.0),
                    fill=color,
                    outline="black",
                ...
                self.canvas.create_rectangle(
                    self.scale_x(x - STEM_WIDTH / 2.0),
                    self.scale_y(y + self.suction_height / 2.0),
                    self.scale_x(x + STEM_WIDTH / 2.0),
                    self.scale_y(y + self.suction_height / 2.0 + STEM_HEIGHT),
                    fill=color,
                    outline="black",
                    width=2,
                ),
