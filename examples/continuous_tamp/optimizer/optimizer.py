...
def distance_cost(model, q1, q2, norm=1):
    ...
    for i in range(len(q1)):
        ...
        if norm == 1:
            ...
            distance = model.addVar(lb=0.0, ub=GRB.INFINITY)
...
def sample_solutions(model, variables, num_samples=INF, norm=2, closest=True):
    ...
    objective_terms = []
    if closest:
        min_distance = model.addVar(lb=0.0, ub=GRB.INFINITY)
    ...
    while len(solutions) < num_samples:
        ...
        for var in variables:
            for index, coord in enumerate(var):
                ...
                if norm == 1:
                    term = model.addVar(lb=0.0, ub=GRB.INFINITY)
        ...
        model.setObjective(
            quicksum(objective_terms), sense=GRB.MAXIMIZE
        )  # MINIMIZE | MAXIMIZE
        model.optimize()
        print(
...
def get_optimize_fn(
    ...
    def fn(outputs, facts, hint={}):
        ...
        linear_constraints = {
            c for c in linear_model.getConstrs() if c.sense == GRB.EQUAL
        }
        ...
        print(
            "{} constraints: (codim={}): {}".format(
                len(nontrivial_indices),
                codimension,
        ...
        if weight > 0:
            ...
            objective_terms.extend(
                weight * term for term in sample_targets(model, primary_variables)
            )
        model.setObjective(
            quicksum(objective_terms), sense=GRB.MINIMIZE
        )  # (1-weight) * quicksum(objective_terms)
        ...
        print(
            "Solved: {} | Objective: {:.3f} | Solutions: {} | Status: {} | Runtime: {:.3f}".format(
                True, model.ObjVal, model.SolCount, model.status, model.runtime
            )
        )

        if costs and diagnose_cost:
            infeasible = deletion_filter(
                model, constraint_indices, max_objective=model.ObjVal - 1e-6
            )
        else:
...
def relax_constraints(model, indices):
    ...
    objective = model.feasRelax(
        relaxobjtype=0,  # feasRelaxS
        minrelax=True,
        vars=[],
        lbpen=[],
        ubpen=[],
        constrs=elastic_constraints,
        rhspen=[1.0] * len(elastic_constraints),
    )
    model.optimize()
    ...
    art_vars = [
        ...
        if any(var.varname.startswith(p) for p in ["ArtP_", "ArtN_"])
    ]  # Positive & Negative
    violations = {}
    for var in art_vars:
