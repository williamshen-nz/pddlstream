    ...
    def test(r, t, b2, p2):
        ...
        for _ in t.apply(state):
            ...
            if pairwise_collision(r, b2):
                ...
                handles = add_line(
                    point_from_conf(t.path[0].values, z=0.02),
                    point_from_conf(t.path[-1].values, z=0.02),
                    color=color,
                )
                wait_for_user()
                set_renderer(False)
    ...
    **kwargs
    ...
    def gen(robot, body):
        ...
        for [scale] in unit_generator(d=1, use_halton=use_halton):
            ...
            position = np.append(
                distance * unit_from_theta(theta=theta), [z]
            )  # TODO: halton
...
def get_sample_gen_fn(problem):
    def gen_fn(robot):
        joints = get_base_joints(robot)
        sample_fn = get_halton_sample_fn(
            robot, joints, custom_limits=problem.custom_limits
        )
        collision_fn = get_collision_fn(
            ...
            custom_limits=problem.custom_limits,
...
def get_cost_fn(problem, constant=0.0, coefficient=1.0):
    def fn(r, q1, q2):
