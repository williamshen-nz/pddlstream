...
from examples.pybullet.namo.stream import (
    ...
    set_base_conf,
    get_custom_limits,
    get_test_cfree_conf_pose,
    get_test_cfree_traj_pose,
    get_grasp_generator,
    get_ik_fn,
    get_motion_fn,
    get_cost_fn,
...
class NAMOProblem(object):
    def __init__(
        ...
        self.obstacles = tuple(
            body for body in get_bodies() if body not in self.robots + self.movable
        )
        self.custom_limits = {}
        ...
        self.initial_confs = {
            robot: Conf(robot, get_base_joints(robot)) for robot in self.robots
        }
...
def problem_fn(n_rovers=1, collisions=True):
    ...
    wall5 = create_box(
        mound_height, (base_extent + mound_height) / 2.0, mound_height, color=GREY
    )
    set_point(wall5, Point(y=base_extent / 4.0, z=mound_height / 2.0))
    ...
    set_point(body1, Point(y=-base_extent / 4.0, z=mound_height / 2.0))
    body2 = create_cylinder(cylinder_radius, mound_height, color=BLUE)
    set_point(
        body2, Point(x=base_extent / 4.0, y=3 * base_extent / 8.0, z=mound_height / 2.0)
    )
    movable = [body1, body2]
...
def pddlstream_from_problem(problem):
    ...
    goal_literals = []
    goal_literals += [
        ("Holding", robot, body) for robot, body in problem.goal_holding.items()
    ]
