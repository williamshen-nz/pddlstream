...
from examples.pybullet.namo.stream import (
    BASE_RESOLUTIONS,
    get_turtle_aabb,
    get_base_joints,
    set_base_conf,
...
def get_test_cfree_traj_traj(problem):
    ...
    def test(traj1, traj2):
        ...
        for conf1 in randomize(traj1.iterate()):
            if not aabb_overlap(
                get_turtle_traj_aabb(conf1), get_turtle_traj_aabb(traj2)
            ):
                continue
            set_base_conf(robot1, conf1.values)
            for conf2 in randomize(traj2.iterate()):
                if not aabb_overlap(
                    get_turtle_traj_aabb(conf1), get_turtle_traj_aabb(conf2)
                ):
                    continue
...
def get_motion_fn2(problem):
    ...
    motion_fn = get_motion_fn(
        problem, max_distance=0.45, weights=WEIGHTS, resolutions=BASE_RESOLUTIONS
    )
...
def draw_edges(edges):
    ...
    for conf1, conf2 in edges:
        handles.append(
            add_line(point_from_conf(conf1.values), point_from_conf(conf2.values))
        )
    return handles
...
def pddlstream_from_problem(problem, teleport=False):
    ...
    constant_map = {
        "{}".format(name).lower(): name for name in problem.initial_confs.keys()
    }
    ...
    with LockRenderer():
        init += [
            ("Conf", q)
            for _, n, q in create_vertices(problem, robot, [], samples_per_ft2=6)
        ]
    ...
    problem = PDDLProblem(
        domain_pddl, constant_map, stream_pddl, stream_map, init, goal_formula
    )
...
class NAMOProblem(object):
    def __init__(self, body_from_name, robots, limits, collisions=True, goal_confs={}):
        ...
        self.obstacles = tuple(
            body for body in get_bodies() if body not in robot_bodies
        )
        self.custom_limits = {}
        ...
        self.initial_confs = {
            name: Conf(self.get_body(name), get_base_joints(self.get_body(name)))
...
def problem_fn(n_robots=2, collisions=True):
    ...
    width = base_extent / 2.0 - 4 * mound_height
    wall5 = create_box(mound_height, width, mound_height, color=GREY)
    set_point(wall5, Point(y=-(base_extent / 2 - width / 2.0), z=mound_height / 2.0))
    wall6 = create_box(mound_height, width, mound_height, color=GREY)
    set_point(wall6, Point(y=+(base_extent / 2 - width / 2.0), z=mound_height / 2.0))
...
def main(teleport=False):
    ...
    parser.add_argument(
        "-algorithm", default="incremental", help="Specifies the algorithm"
    )
    parser.add_argument("-cfree", action="store_true", help="Disables collisions")
    parser.add_argument(
        "-deterministic", action="store_true", help="Uses a deterministic sampler"
    )
    parser.add_argument("-optimal", action="store_true", help="Runs in an anytime mode")
    parser.add_argument(
        "-t", "--max_time", default=5 * 60, type=int, help="The max time"
    )
    parser.add_argument(
        "-enable", action="store_true", help="Enables rendering during planning"
    )
    parser.add_argument(
        "-viewer", action="store_true", help="Enable the viewer and visualizes the plan"
    )
