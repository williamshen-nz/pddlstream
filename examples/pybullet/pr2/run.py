...
def pddlstream_from_problem(problem, collisions=True, teleport=False):
    ...
    stream_map = {
        ...
        "inverse-kinematics": from_gen_fn(
            get_ik_ir_gen(problem, collisions=collisions, teleport=teleport)
        ),
        "plan-base-motion": from_fn(
            get_motion_gen(problem, collisions=collisions, teleport=teleport)
        ),
        ...
        "test-cfree-traj-pose": from_test(
            get_cfree_traj_pose_test(problem.robot, collisions=collisions)
        ),
...
def post_process(problem, plan, teleport=False):
    ...
    for i, (name, args) in enumerate(plan):
        if name == "move_base":
            ...
            a, b, p, g, _, c = args
            [t] = c.commands
            close_gripper = GripperCommand(
                problem.robot, a, g.grasp_width, teleport=teleport
            )
...
def main(partial=False, defer=False, verbose=True):
    ...
    parser.add_argument(
        "-cfree", action="store_true", help="Disables collisions during planning"
    )
    parser.add_argument(
    ...
    stream_info.update(
        {
            ...
            "plan-base-motion": StreamInfo(
                opt_gen_fn=PartialInputs("?q1 ?q2"),
