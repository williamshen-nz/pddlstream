...
from examples.pybullet.turtlebot_rovers.problems import (
    PROBLEMS,
    get_base_joints,
    KINECT_FRAME,
)
from examples.pybullet.turtlebot_rovers.streams import (
    get_reachable_test,
    get_inv_vis_gen,
    get_inv_com_gen,
    get_above_gen,
...
def pddlstream_from_problem(problem, collisions=True, **kwargs):
    ...
    goal_literals = [
        ...
        Exists(
            ["?rock"], And(("Type", "?rock", "stone"), ("ReceivedAnalysis", "?rock"))
        ),
        Exists(
            ["?soil"], And(("Type", "?soil", "soil"), ("ReceivedAnalysis", "?soil"))
        ),
    ]
    ...
    stream_map = {
        "test-cfree-ray-conf": from_test(
            get_cfree_ray_test(problem, collisions=collisions)
        ),
        "test-reachable": from_test(
            get_reachable_test(
...
def main():
    ...
    parser.add_argument(
        "-problem", default="rovers1", help="The name of the problem to solve"
    )
