...
from examples.pybullet.tamp.streams import (
    ...
    get_cfree_traj_grasp_pose_test,
    BASE_CONSTANT,
    distance_fn,
    move_cost_fn,
)
...
def pddlstream_from_problem(problem, base_limits=None, collisions=True, teleport=False):
    ...
    stream_map = {
        ...
        "inverse-kinematics": from_gen_fn(
            get_ik_ir_gen(
                problem,
        ...
        "test-cfree-approach-pose": from_test(
            get_cfree_approach_pose_test(problem, collisions=collisions)
        ),
        "test-cfree-traj-pose": from_test(
            get_cfree_traj_pose_test(robot, collisions=collisions)
        ),
...
def main(verbose=True):
    ...
    parser.add_argument(
        "-problem", default="packed", help="The name of the problem to solve"
    )
    parser.add_argument(
        "-n", "--number", default=5, type=int, help="The number of objects"
    )
    ...
    stream_info = {
        ...
        "test-cfree-traj-pose": StreamInfo(
            p_success=1e-1, verbose=verbose
        ),  # TODO: apply to arm and base trajs
