...
from examples.pybullet.pr2_rovers.streams import (
    get_reachable_test,
    get_inv_vis_gen,
    get_inv_com_gen,
    get_above_gen,
...
def pddlstream_from_problem(problem, collisions=True, teleport=False):
    ...
    stream_map = {
        ...
        "obj-inv-visible": from_gen_fn(
            get_inv_vis_gen(problem, custom_limits=custom_limits)
        ),
        "com-inv-visible": from_gen_fn(
            get_inv_com_gen(problem, custom_limits=custom_limits)
        ),
        "sample-above": from_gen_fn(
            get_above_gen(problem, custom_limits=custom_limits)
        ),
        "sample-base-motion": from_fn(
            get_base_motion_fn(problem, custom_limits=custom_limits, teleport=teleport)
        ),
...
def main():
    ...
    parser.add_argument(
        "-problem", default="problem1", help="The name of the problem to solve"
    )
    ...
    pddlstream_problem = pddlstream_from_problem(
        problem, collisions=not args.cfree, teleport=args.teleport
    )
    stream_info = {
