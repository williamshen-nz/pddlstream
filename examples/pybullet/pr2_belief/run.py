...
from examples.pybullet.pr2_belief.primitives import (
    Scan,
    ScanRoom,
    Detect,
    Register,
...
from examples.pybullet.pr2_belief.problems import (
    get_problem1,
    USE_DRAKE_PR2,
    create_pr2,
)
...
def pddlstream_from_state(state, teleport=False):
    ...
    for attach in state.attachments.values():
        ...
        init += [
            ("Grasp", attach.body, attach.grasp),
    ...
    goal = And(
        *[("Holding", a, b) for a, b in task.goal_holding]
    ...
    stream_map = {
        ...
        "sample-vis-base": accelerate_list_gen_fn(
            from_gen_fn(get_vis_base_gen(task, VIS_RANGE)), max_attempts=25
        ),
        "sample-reg-base": accelerate_list_gen_fn(
            from_gen_fn(get_vis_base_gen(task, REG_RANGE)), max_attempts=25
        ),
...
def plan_commands(state, args, profile=True, verbose=True):
    ...
    mapping = clone_world(
        client=sim_world, exclude=[task.robot]
