...
class DiscreteTAMPViewer(object):
    ...
    def transform_r(self, r):
        return (
            self.table_y1
            + r * (self.side + 2 * self.block_buffer)
            + 2 * self.block_buffer
            + self.side / 2
        )
    ...
    def transform_c(self, c):
        ...
        return (
            self.table_x1
            + c * (self.side + 2 * self.block_buffer)
            + 2 * self.block_buffer
            + self.side / 2
        )
    ...
    def draw_environment(self, table_color="lightgrey", bin_color="grey"):
        table_width = (
            self.cols * (self.side + 2 * self.block_buffer) + 2 * self.block_buffer
        )
        table_height = (
            self.rows * (self.side + 2 * self.block_buffer) + 2 * self.block_buffer
        )
