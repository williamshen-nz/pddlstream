...
def get_extend_fn(joints, resolutions=None):
    ...
    def fn(q1, q2):
        num_steps = (
            int(np.max(np.abs(np.divide(difference_fn(q2, q1), resolutions)))) + 1
        )
        q = q1
...
def get_collision_fn(
    ...
    def fn(q):
        if any(
            not within_limits(joint, position) for joint, position in zip(joints, q)
        ):
            return True
        ...
        return exists_colliding_pair(
            diagram, diagram_context, plant, scene_graph, collision_pairs
        )
    ...
    **kwargs
    ...
    for frame_pose in frame_path:
        solution = solve_inverse_kinematics(
            plant, frame, frame_pose, initial_guess=solution, **kwargs
        )
        if solution is None:
            return None
...
def smooth_joint_motion(
    joints, path, resolutions=None, collision_fn=lambda q: False, **kwargs
):
    return smooth_path(
        path,
