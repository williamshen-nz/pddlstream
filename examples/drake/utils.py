...
def vertices_from_aabb(aabb):
    center, extent = aabb
    return [
        center + np.multiply(extent, np.array(signs))
...
def aabb_from_points(points):
    ...
    extent = (np.array(upper) - np.array(lower)) / 2.0
...
def get_model_aabb(mbp, context, box_from_geom, model_index):
    ...
    for (model_int, body_name, _), (aabb, body_from_geom, _) in box_from_geom.items():
        if (int(model_index) == model_int) and (body_name in body_names):
            ...
            points.extend(
                world_from_body.multiply(body_from_geom).multiply(vertex)
...
def get_model_actuators(mbp, model_index):
    return [
        actuator
        for actuator in get_joint_actuators(mbp)
        if actuator.model_instance() == model_index
    ]
...
def get_parent_joints(mbp, body):
    ...
    return [
        joint
        for joint in get_movable_joints(mbp, body.model_instance())
...
def dump_plant(mbp):
    ...
    for i, body in enumerate(get_bodies(mbp)):
        print(
            "{}) {} {}: {}".format(
                i, body.__class__.__name__, body.name(), body.body_frame().name()
            )
        )
    print("\nJoints:")
...
def dump_model(mbp, model_index):
    print(
        "\nModel {}: {}".format(
            int(model_index), mbp.tree().GetModelInstanceName(model_index)
        )
    )
...
def solve_inverse_kinematics(
    ...
    ik_scene.AddOrientationConstraint(
        frameAbar=target_frame,
        R_AbarA=RotationMatrix.Identity(),
        frameBbar=world_frame,
        R_BbarB=RotationMatrix(target_pose.rotation()),
...
def get_colliding_bodies(
    diagram, diagram_context, plant, scene_graph, min_penetration=0.0
):
...
def exists_colliding_pair(
    diagram, diagram_context, plant, scene_graph, body_pairs, **kwargs
):
    if not body_pairs:
        return False
    intersection = (
        get_colliding_bodies(diagram, diagram_context, plant, scene_graph, **kwargs)
        & body_pairs
    )
    # if intersection:
...
def get_box_from_geom(scene_graph, visual_only=True):
    ...
    for body_index in range(load_robot_msg.num_links):
        ...
        visual_index = 0
        for geom in sorted(
            link.geom, key=lambda g: g.position[::-1]
        ):  # sort by z, y, x
            ...
            if geom.type == geom.BOX:
                ...
                extent = np.array([width, length, height]) / 2.0
                ...
                extent = np.array([radius, radius, height / 2.0])
            ...
            ).GetAsIsometry3()  # .GetAsMatrix4()
            box_from_geom[model_index, frame_name, visual_index - 1] = (
                BoundingBox(np.zeros(3), extent),
                link_from_box,
                get_geom_name(geom),
            )
