...
def build_manipulation_station(station):
    ...
    builder.Connect(
        plan_runner.hand_setpoint_output_port, station.GetInputPort("wsg_position")
    )
    builder.Connect(
        plan_runner.gripper_force_limit_output_port,
    ...
    builder.Connect(demux.get_output_port(0), station.GetInputPort("iiwa_position"))
    builder.Connect(
        demux.get_output_port(1), station.GetInputPort("iiwa_feedforward_torque")
    )
    builder.Connect(
    ...
    viz = MeshcatVisualizer(scene_graph, is_drawing_contact_force=True, plant=plant)
    builder.AddSystem(viz)
    builder.Connect(
        station.GetOutputPort("pose_bundle"), viz.GetInputPort("lcm_visualization")
    )
    builder.Connect(
        station.GetOutputPort("contact_results"), viz.GetInputPort("contact_results")
    )
    ...
    wsg_state_log = LogOutput(station.GetOutputPort("wsg_state_measured"), builder)
...
def add_drake_visualizer(scene_graph, builder):
    ...
    DispatchLoadMessage(
        scene_graph, lcm
    )  # TODO: only update viewer after a plan is found
...
def connect_controllers(builder, mbp, robot, gripper, print_period=1.0):
    from examples.drake.manipulation_station.kuka_multibody_controllers import (
        KukaMultibodyController,
        HandController,
        ManipStateMachine,
    )

    iiwa_controller = KukaMultibodyController(
        plant=mbp, kuka_model_instance=robot, print_period=print_period
    )
    builder.AddSystem(iiwa_controller)

    builder.Connect(
        iiwa_controller.get_output_port(0), mbp.GetInputPort("iiwa_actuation")
    )
    builder.Connect(
        mbp.get_continuous_state_output_port(), iiwa_controller.robot_state_input_port
    )

    hand_controller = HandController(plant=mbp, model_instance=gripper)
    builder.AddSystem(hand_controller)
    builder.Connect(
        hand_controller.get_output_port(0), mbp.GetInputPort("gripper_actuation")
    )
    builder.Connect(
        mbp.get_continuous_state_output_port(), hand_controller.robot_state_input_port
    )

    state_machine = ManipStateMachine(mbp)
    builder.AddSystem(state_machine)
    builder.Connect(
        mbp.get_continuous_state_output_port(), state_machine.robot_state_input_port
    )
    builder.Connect(
        state_machine.kuka_plan_output_port, iiwa_controller.plan_input_port
    )
    builder.Connect(
        state_machine.hand_setpoint_output_port, hand_controller.setpoint_input_port
    )
