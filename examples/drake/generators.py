...
from examples.drake.iiwa_utils import (
    ...
    get_close_wsg50_positions,
    get_open_wsg50_positions,
)
from examples.drake.motion import (
    plan_joint_motion,
...
class Pose(object):
    ...
    @property
    def bodies(self):
        # Returns BodyIndex of a given child model index (I don't really get it...)
...
class Trajectory(object):
    ...
    def spline(self):
        ...
        distances = [0.0] + [distance_fn(q1, q2) for q1, q2 in zip(path, path[1:])]
        # TODO: increase time for pick/place & hold
        t_knots = (
            np.cumsum(distances) / RADIANS_PER_SECOND
        )  # TODO: this should be a max
...
def get_open_trajectory(plant, gripper):
    ...
    gripper_path = list(
        gripper_extend_fn(gripper_closed_conf, get_open_wsg50_positions(plant, gripper))
    )
...
def get_pose_gen(task, context, collisions=True, shrink=0.025):
    ...
    def gen(obj_name, surface):
        ...
        for surface_box_from_obj_box in sample_aabb_placement(
            obj_aabb, surface_aabb, shrink=shrink
        ):
            world_pose = (
                surface_pose.multiply(surface_from_box)
                .multiply(surface_box_from_obj_box)
                .multiply(obj_from_box.inverse())
            )
            pose = Pose(mbp, world, obj, world_pose)
            pose.assign(context)
            if not exists_colliding_pair(
                task.diagram,
...
def plan_frame_motion(
    ...
    waypoints = plan_workspace_motion(
        plant,
        joints,
        frame,
        frame_path,
    ...
    return plan_waypoints_joint_motion(
        joints, waypoints, resolutions=resolutions, collision_fn=collision_fn
    )
...
def get_ik_gen_fn(
    ...
    def fn(robot_name, obj_name, obj_pose, obj_grasp):
        ...
        collision_pairs = set(
            product(bodies_from_models(task.mbp, [robot, task.gripper]), fixed)
        )
        ...
        gripper_path = list(
            interpolate_translation(gripper_pose, approach_vector, step_size=step_size)
        )
...
def get_reachable_pose_gen_fn(task, context, collisions=True, **kwargs):
    ...
    ik_gen_fn = get_ik_gen_fn(
        task, context, collisions=collisions, max_failures=1, **kwargs
    )
...
def get_door_grasp(door_body, box_from_geom):
    ...
    grasp_length = 0.02
    target_shape, target_ori = (
        "cylinder",
        1,
    )  # Second grasp is np.pi/2, corresponding to +y
    # target_shape, target_ori = 'box', 0 # left_door TODO: right_door
    for i in range(2):
        handle_aabb, handle_from_box, handle_shape = box_from_geom[
            int(door_body.model_instance()), door_body.name(), i
        ]
        if handle_shape == target_shape:
            break
    else:
        raise RuntimeError(target_shape)
    [gripper_from_box] = list(
        get_box_grasps(
            handle_aabb,
...
def get_pull_fn(
    ...
    def fn(robot_name, door_name, door_conf1, door_conf2):
        ...
        extend_fn = get_extend_fn(
            door_joints, resolutions=step_size * np.ones(len(door_joints))
        )
