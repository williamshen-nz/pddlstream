...
from examples.drake.generators import (
    Pose,
    Conf,
    get_grasp_gen_fn,
    get_ik_gen_fn,
...
def get_pddlstream_problem(task, context, collisions=True):
    ...
    for obj in task.movable:
        ...
        obj_pose = Pose(
            plant, world, obj, get_world_pose(plant, context, obj)
    ...
    stream_map = {
        ...
        "sample-reachable-pose": from_gen_fn(
...
def main(deterministic=False):
    ...
    parser.add_argument(
        "-p",
        "--problem",
        default="load_manipulation",
        help="The name of the problem to solve",
    )
    parser.add_argument(
        "-c", "--cfree", action="store_true", help="Disables collisions when planning"
    )
    parser.add_argument(
        "-v", "--visualizer", action="store_true", help="Use the drake visualizer"
    )
    parser.add_argument(
        "-s", "--simulate", action="store_true", help="Simulates the system"
    )
    ...
    task, diagram, state_machine = problem_fn(
        time_step=time_step,
        use_meshcat=not args.visualizer,
    ...
    if args.simulate:
        ...
        splines, gripper_setpoints = convert_splines(
            plant, task.robot, task.gripper, context, trajectories
        )
        ...
        step_trajectories(
            diagram, diagram_context, context, trajectories, time_step=time_step
        )  # , teleport=True)
