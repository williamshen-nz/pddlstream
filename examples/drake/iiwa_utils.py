...
def close_wsg50_gripper(mbp, context, model_index):  # 0.05
    set_joint_positions(
        get_movable_joints(mbp, model_index),
        context,
...
TOOL_Z = (
    0.025 + 0.075 / 2 - (-0.049133)
)  # Difference between WSG50 finger tip and body link
DEFAULT_LENGTH = 0.03
# DEFAULT_LENGTH = 0.0
...
def get_top_cylinder_grasps(
    aabb, max_width=DEFAULT_MAX_WIDTH, grasp_length=DEFAULT_LENGTH
):  # y is out of gripper initially
    center, extent = aabb
    w, l, h = 2 * extent
...
def get_box_grasps(
    ...
    if min_pitch == max_pitch:
        ...
        generator = (
            (random.uniform(min_pitch, max_pitch), random.choice(orientations))
            for _ in count()
        )
