...
def load_station(time_step=0.0, **kwargs):
    ...
    goal_poses = {
        brick: create_transform(
            translation=[0.8, 0.2, 0.2927], rotation=[0, 0, 5 * np.pi / 4]
        ),
    }
...
def load_manipulation(
    time_step=0.0, use_meshcat=True, use_controllers=True, use_external=False
):
    if not use_external:
    ...
    cupboard_z = (
        dz_cupboard_to_table_center + cupboard_height / 2.0 - dz_table_top_robot_base
    )

    robot = AddModelFromSdfFile(
        file_name=IIWA7_PATH, model_name="iiwa", scene_graph=scene_graph, plant=plant
    )
    gripper = AddModelFromSdfFile(
        file_name=WSG50_SDF_PATH,
        model_name="gripper",
        scene_graph=scene_graph,
        plant=plant,
    )  # TODO: sdf frame/link error
    table = AddModelFromSdfFile(
        file_name=AMAZON_TABLE_PATH,
        model_name="table",
        scene_graph=scene_graph,
        plant=plant,
    )
    cupboard = AddModelFromSdfFile(
        file_name=CUPBOARD_PATH,
        model_name="cupboard",
        scene_graph=scene_graph,
        plant=plant,
    )
    brick = AddModelFromSdfFile(
        file_name=FOAM_BRICK_PATH,
        model_name="brick",
        scene_graph=scene_graph,
        plant=plant,
    )
    ...
    goal_poses = {
        brick: create_transform(
            translation=[0.8, 0.2, 0.2927], rotation=[0, 0, np.pi / 8]
        ),
    }
    goal_holding = [
        # brick,
    ...
    diagram, state_machine = build_diagram(
        plant,
        scene_graph,
        robot,
        gripper,
...
def load_tables(time_step=0.0, use_meshcat=True, use_controllers=True):
    ...
    table = AddModelFromSdfFile(
        file_name=TABLE_SDF_PATH,
        model_name="table",
        scene_graph=scene_graph,
        plant=plant,
    )
    table2 = AddModelFromSdfFile(
        file_name=TABLE_SDF_PATH,
        model_name="table2",
        scene_graph=scene_graph,
        plant=plant,
    )
    sink = AddModelFromSdfFile(
        file_name=SINK_PATH, model_name="sink", scene_graph=scene_graph, plant=plant
    )
    stove = AddModelFromSdfFile(
        file_name=STOVE_PATH, model_name="stove", scene_graph=scene_graph, plant=plant
    )
    broccoli = AddModelFromSdfFile(
        file_name=BROCCOLI_PATH,
        model_name="broccoli",
        scene_graph=scene_graph,
        plant=plant,
    )
    ...
    weld_to_world(
        plant, sink, create_transform(translation=[table2_x, 0.25, table_top_z])
    )
    weld_to_world(
        plant, stove, create_transform(translation=[table2_x, -0.25, table_top_z])
    )
    if wall is not None:
        weld_to_world(
            plant, wall, create_transform(translation=[table2_x / 2, 0, table_top_z])
        )
    plant.Finalize(scene_graph)
