...
class Plan:
    def __init__(self, type=None, trajectory=None, start_time=None, R_WE_ref=None):
        self.type = type
...
class KukaMultibodyController(LeafSystem):
    def __init__(
        self, plant, kuka_model_instance, control_period=0.005, print_period=0.5
    ):
        ...
        self.controlled_inds = self.tree.get_velocities_from_array(
            kuka_model_instance, v_dummy
        ).astype(int)

        self.robot_state_input_port = self._DeclareInputPort(
            PortDataType.kVectorValued, plant.num_positions() + plant.num_velocities()
        )
        self.plan_input_port = self._DeclareInputPort(PortDataType.kAbstractValued, 0)

        self._DeclareDiscreteState(self.nu)
        self._DeclarePeriodicDiscreteUpdate(period_sec=control_period)
        self._DeclareVectorOutputPort(BasicVector(self.nu), self._DoCalcVectorOutput)
    ...
    def _DoCalcDiscreteVariableUpdates(self, context, events, discrete_state):
        ...
        x = self.EvalVectorInput(
            context, self.robot_state_input_port.get_index()
        ).get_value()
        plan = self.EvalAbstractInput(
            context, self.plan_input_port.get_index()
        ).get_value()
        q = x[: self.nq]
        v = x[self.nq :]
        ...
        if plan.type == "JointSpacePlan":
            ...
            qDDt_desired[self.controlled_inds] = 1000.0 * qerr_kuka + 100 * verr_kuka

        lhs = tree.CalcInverseDynamics(
            context=context,
    ...
    def _DoCalcVectorOutput(self, context, y_data):
        if (
            self.print_period
            and context.get_time() - self.last_print_time >= self.print_period
        ):
...
class HandController(LeafSystem):
    def __init__(self, plant, model_instance, control_period=0.001):
        ...
        self.max_force = 100.0  # gripper max closing / opening force
        self.plant = plant
        ...
        self.setpoint_input_port = self._DeclareInputPort(PortDataType.kVectorValued, 1)
    ...
    def _DoCalcDiscreteVariableUpdates(self, context, events, discrete_state):
        ...
        gripper_width_des = self.EvalVectorInput(
            context, self.setpoint_input_port.get_index()
        ).get_value()
...
class ManipStateMachine(LeafSystem):
    ...
    def __init__(self, plant):
        ...
        self.kuka_plan_output_port = self._DeclareAbstractOutputPort(
            lambda: AbstractValue.Make(Plan()), self.CalcPlan
        )
        self.hand_setpoint_output_port = self._DeclareVectorOutputPort(
    ...
    def Load(self, qtraj_list, gripper_setpoint_list):
        ...
        self.current_plan = Plan(
            type=plan_types[0], trajectory=self.qtraj_list[0], start_time=0
        )
        self.loaded = True
