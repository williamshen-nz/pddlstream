...
class JointSpacePlan(PlanBase):
    def __init__(self, trajectory=None):
        PlanBase.__init__(self, type=PlanTypes[0], trajectory=trajectory)
...
# theta0_hinge = np.arctan2(np.abs(p_handle_2_hinge[0]), np.abs(p_handle_2_hinge[1]))
#
# # position of point Q in EE frame.  Point Q is fixed in the EE frame.
# p_EQ = GetEndEffectorWorldAlignedFrame().multiply(np.array([0.0, 0.0, 0.095]))
#
...
#         angle_traj = PiecewisePolynomial.Cubic(
#             t_knots, door_angle_knots.T, np.zeros(1), np.zeros(1)
#         )
...
#                 -r_handle * np.cos(handle_angle_ref),
#                 0,
#             ]
#         )
#         X_HrW = X_WHr.inverse()
...
#             context_iiwa, frame_A=world_frame, frame_B=l7_frame
#         )
#
...
#             v_ee_desired = np.zeros(
#                 6
#             )  # first 3: angular velocity, last 3: translational velocity
#             kp_translation = np.array([50.0, 5.0, 50]) / 2
...
#         if t_plan < self.duration:
#             f_ee_desired = np.zeros(
#                 6
#             )  # first 3: angular velocity, last 3: translational velocity
#             kp_translation = np.array([50.0, 5.0, 50])
...
#     def __init__(self, trajectory=None, R_WE_ref=None):
#         PlanBase.__init__(self, type=PlanTypes[1], trajectory=trajectory)
...
#     def __init__(self, trajectory=None):
#         PlanBase.__init__(self, type=PlanTypes[2], trajectory=trajectory)
#
#
# """
...
#     def __init__(self, x_ee_traj=None, f_ee_traj=None, selector=None):
#         PlanBase.__init__(self, type=PlanTypes[3], trajectory=x_ee_traj)
