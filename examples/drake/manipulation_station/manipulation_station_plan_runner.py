...
class ManipStationPlanRunner(LeafSystem):
    ...
    def __init__(self, station, control_period=0.005, print_period=0.5):
        ...
        self.iiwa_position_input_port = self._DeclareInputPort(
            "iiwa_position", PortDataType.kVectorValued, 7
        )
        self.iiwa_velocity_input_port = self._DeclareInputPort(
        ...
        self.iiwa_position_command_output_port = self._DeclareVectorOutputPort(
            "iiwa_position_and_torque_command",
            BasicVector(self.nu * 2),
            self.CalcIiwaCommand,
        )
        ...
        self.gripper_force_limit_output_port = self._DeclareVectorOutputPort(
    ...
    def Load(self, kuka_plans, gripper_setpoint_list):
        ...
        for i in range(2, self.num_plans):
            self.t_plan[i + 1] = self.t_plan[i] + kuka_plans[i].get_duration() * 1.1
    ...
    def CalcIiwaCommand(self, context, y_data):
        ...
        if self.current_plan.type == "JointSpacePlan":
            ...
            self.current_plan.type == "OpenLeftDoorImpedancePlan"
            or self.current_plan.type == "OpenLeftDoorPositionPlan"
        ):
            q_iiwa = self.EvalVectorInput(
                context, self.iiwa_position_input_port.get_index()
            ).get_value()
            # update self.context_iiwa
            x_iiwa_mutable = self.tree_iiwa.get_mutable_multibody_state_vector(
                self.context_iiwa
            )
            ...
            Jv_WL7q, p_HrQ = self.current_plan.CalcKinematics(
                ...
                tree_iiwa=self.tree_iiwa,
                context_iiwa=self.context_iiwa,
        ...
        if self.print_period and t - self.last_print_time >= self.print_period:
    ...
    def GetCurrentPlan(self, context):
        ...
        if self.kuka_plans_list[0].traj is None:
            ...
            t_knots = np.array(
                [
                    0.0,
                    self.move_to_home_duration_sec / 2,
            ...
            self.current_plan.set_start_time(0.0)
