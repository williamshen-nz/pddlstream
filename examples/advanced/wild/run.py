...
def compute_split(pair, s):
    x1, x2 = pair
    return [(x1, convex_combo(x1, x2, s)), (convex_combo(x1, x2, s), x2)]


def compute_cost(pair, s):
    x1, x2 = pair
    return round(
        1.0 / (min(convex_combo(x1, x2, s) - x1, x2 - convex_combo(x1, x2, s))), 3
    )
...
def get_problem(num=3):
    ...
    goal = And(
        *[Exists(["?c"], And(("Loc", loc), ("AtLoc", "?c", loc))) for loc in locations]
    )
...
def main():
    ...
    solution = solve(
        ...
        stream_info=info,
        effort_weight=1,  # success_cost=0., max_iterations=3, max_time=5,
        debug=False,
        verbose=True,
    )
