...
def create_problem1():
    ...
    info = {
        ...
        streams[2]: StreamInfo(p_success=0.0),
...
def create_problem2(chain1=2, chain2=2):
    ...
    orders = set(get_pairs(streams[:chain1])) | set(get_pairs(streams[chain1:]))
    # info = {
    ...
    # ]
    info = [
        {"p_success": 1.0, "max_attempts": INF},
        {"p_success": 0.2, "max_attempts": 1},
        {"p_success": 1.0, "max_attempts": INF},
        {"p_success": 0.0, "max_attempts": INF},
    ]
...
def outcome_generator(p_success=1.0, start_index=1, stop_index=INF):
...
def get_gen_fn(outputs=[], p_success=1.0, max_attempts=INF):
    history = []
    ...
    def gen_fn(*args, **kwargs):
        for _ in irange(max_attempts):
            if random.random() < p_success:
                values = [
                    "{}-{}".format(output[1:], len(history)) for output in outputs
                ]
...
def opt_from_graph(names, orders, infos={}):
    ...
    object_from_param = {
        param: parse_value(param) for param in param_from_order.values()
    }
...
def solve_skeleton(
    ...
    for opt_solution in opt_solutions:
        ...
        skeleton_queue.process(
            stream_plan, opt_plan, cost, complexity_limit=max_complexity, max_time=0
        )
